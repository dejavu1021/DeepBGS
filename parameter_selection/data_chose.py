...
for train_index, test_index in kfold.split(train_data, train_lable,):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=13, shuffle=True)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=13, shuffle=True)
    ...
    with torch.no_grad():
        ...
        for features, labels in test_loader:
            ...
            all[b,0, total:total + 13] = outputs_softmax[:, 1]
            all[b,1, total:total + 13] = predicted
            all[b,2, total:total + 13] = labels
...
np.savetxt("train3_7.csv", alll, delimiter=",")
