...
def CARS_Cloud(X, y, N=50, f=20, cv=10):
    ...
    plt.scatter(MinIndex, len(OptWave), marker='s', color='r')
    print(WaveNum[MinIndex])
    plt.savefig('crasss101.png', dpi=800)

    fig2 = plt.figure(figsize=(8, 7.2))
    # plt.subplot(212)
    plt.xlabel('蒙特卡洛迭代次数', fontsize=fonts)
    plt.ylabel('RMSECV', fontsize=fonts)
    plt.title('最佳迭代次数：' + str(MinIndex) + '次', fontsize=fonts)
    plt.plot(np.arange(N), RMSECV)
    plt.scatter(MinIndex,np.min(RMSECV),marker='s', color='r')
    print(np.min(RMSECV))
    plt.savefig('crasss102.png', dpi=800)

    # fig3 = plt.figure(8, 4.8)
    # plt.subplot(313)
    # plt.xlabel('蒙特卡洛迭代次数', fontsize=fonts)
