...
def train_model(model, epochs):
    for epoch  in range(1, epochs+1):
        ...
        for features, labels in dl_train:
            ...
            al_train[0, to:to + 10] = outputs_softmax[:, 1]
            al_train[1, to:to + 10] = predicted
            al_train[2, to:to + 10] = labe
            # print(al_train[2])
            to += labe.size(0)
...
# print(all)
# np.savetxt(f"res1.csv", all, delimiter=",")

print(a)
print(sum(a) / len(a))
da = torch.tensor(a)
# np.savetxt("computer_res1.csv", da, delimiter=",")