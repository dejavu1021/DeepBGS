...
for h in range(10):
    ...
    train_acc = []
    train_mcc = []
    test = np.empty((165,2150))
    testi = np.empty((1,165))
    sss = ss[3]
    all = np.empty((3, 165))
...
# 读取CSV数据
    ...
    # train_data, test_data = x[:352], x[352:]
    # train_lable, test_lable = y[:352], y[352:]
    test_data, train_data = x[:165], x[165:]
    train_lable =  y[165:]

    test_index=np.arange(165)
    test_lable = np.hstack((lab[:165].reshape(-1,1),test_index.reshape(-1,1)))
    ...
    # np.savetxt(f"tranmcc{h}.csv", tramcc, delimiter=",")
    model.eval()
    with torch.no_grad():
        correct = 0
        total = 0
        for features, labels in dl_test:
            features = features.to(device)
            testi[0,total:total + batch_size] = labels[:,1].reshape(batch_size)
            ...
            test[total:total + batch_size]= features1[testi[0,total:total + batch_size]]
            # print(testi)
            # print(predicted)
            # print(all[2])
            # print(labels)
    ...
    # print(all)
    np.savetxt(f"lstm/net2_1{h}.csv", all, delimiter=",")
    np.savetxt(f"lstm/test1_{h}.csv", test, delimiter=",")

print(a)
print(sum(a) / len(a))
da = torch.tensor(a)
np.savetxt("lstm/computernet2_1.csv", da, delimiter=",")
