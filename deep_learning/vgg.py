...
# 读取CSV数据
    ...
    lo = torch.tensor(c)
    np.savetxt(f"lossvgg8.csv", lo, delimiter=",")
    # traacc = torch.tensor(train_acc)
    np.savetxt(f"tranaccvgg8.csv", train_acc, delimiter=",")
    # tramcc = torch.tensor(train_mcc)
    np.savetxt(f"tranmccvgg8.csv", train_mcc, delimiter=",")
    ...
    # print(all)
    np.savetxt(f"vgg8.csv", all, delimiter=",")
